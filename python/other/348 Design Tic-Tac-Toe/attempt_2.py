...
class TicTacToe:
    ...
    def move(self, row: int, col: int, player: int) -> int:
        ...
        self.column[col] += current_player
        ...
        if abs(self.rows[row]) == self.target or abs(self.column[col]) == self.target or abs(self.diagonal) == self.target or abs(self.anti_diagonal) == self.target:
            return player
